...
const DEFAULT_VALIDATION_ERRORS = [
    'required' => 'Please enter the %s <hr>',
    'email' => 'The %s is not a valid email address <hr>',
    'min' => 'The %s must have at least %s characters <hr>',
    'max' => 'The %s must have at most %s characters <hr>',
    'between' => 'The %s must have between %d and %d characters <hr>',
    'match' => 'The %s must match with %s <hr>',
    'alphanumeric' => 'The %s should have only letters and numbers <hr>',
    'secure' => 'The %s must have between 8 and 64 characters and contain at least one number, one upper case letter, one lower case letter and one special character <hr>',
    'exists' => 'The %s already exists',
];
...
function is_match(array $data, string $field, string $other): bool
{
...
function is_alpha(array $data, string $field): bool
{
    if (!isset($data[$field])) {
        return false;
    }
...
function is_exists_in_array(array $data, string $field, ...$option): bool
{
    if (!isset($data[$field])) {
        return false;
    }
    $str = (preg_replace('/(\[|\]|\"){1}/', '',  implode(',', $option)));
    $options = explode(',', $str);
    // var_dump($options);
...
function is_exists(array $data, string $field, $table): bool
{
    $tableData = new $table();
    if ($tableData->where($field, $data[$field])) {
        return false;
    } else {
        return true;
    }
