	...
	<form method="post">
		<div class="p-4 mx-auto mr-4 shadow rounded" style="margin-top: 50px;width:100%;max-width: 340px;">
			...
			<?php if ($mode == 'students') : ?>
				<input type="hidden" value="student" name="level">
				<input class="form-control" type="text" disabled value="student" name="level">
			<?php else : ?>
				<select class="my-2 form-control" name="level">
					<option <?= get_select('level', '') ?> value="">--Select a level--</option>
					<option <?= get_select('level', 'student') ?> value="student">Student</option>
					<option <?= get_select('level', 'reception') ?> value="reception">Reception</option>
					<option <?= get_select('level', 'lecturer') ?> value="lecturer">Lecturer</option>
					<option <?= get_select('level', 'admin') ?> value="admin">Admin</option>

					<?php if (Auth::getlevel() == 'super_admin') : ?>
						<option <?= get_select('level', 'super_admin') ?> value="super_admin">Super Admin</option>
					<?php endif; ?>
