...
class Single_class extends Controller
{
	...
	public function index($id = '')
	{
		...
		if ($page_tab == 'lecturers') {
			...
			//display lecturers
			$query = "select class_lecturers.* ,users.* from class_lecturers INNER JOIN users ON class_lecturers.user_id = users.user_id where class_id = :class_id && disabled = 0 ";
		...
		if ($page_tab == 'students') {
			...
			//display lecturers
			$query = "select class_students.* ,users.* from class_students INNER JOIN users ON class_students.user_id = users.user_id where class_id = :class_id && disabled = 0";
		...
		if (count($_POST) > 0) {
			...
			if (isset($_POST['search'])) {
				...
				if (trim($_POST['name']) != "") {
					...
					$query = "select * from users where (firstname like :fname || lastname like :lname) && level = 'lecturer' limit 10";
		...
		if (count($_POST) > 0) {
			...
			if (isset($_POST['search'])) {
				...
				if (trim($_POST['name']) != "") {
					...
					$user = new User();
					$name = "%" . trim($_POST['name']) . "%";
					$query = "select * from users where (firstname like :fname || lastname like :lname) && level = 'student' limit 10";
					$results = $user->query($query, ['fname' => $name, 'lname' => $name,]);
				} else {
					$errors[] = "please type a name to find";
				}
